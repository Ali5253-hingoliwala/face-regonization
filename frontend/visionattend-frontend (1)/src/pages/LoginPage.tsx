  ...
  async function handleGoogle(credential: string) {
    ...
    try {
      ...
      if (result.onboardingRequired) {
        ...
        navigate("/signup?mode=google");
        return;
      }
      navigate(result.role === "admin" ? "/admin" : "/student");
    } catch (err: any) { setError(err?.response?.data?.detail ?? "Google Sign-In failed. Please try again."); }
    finally { setLoading(false); }
  }
    ...
    <div className="min-h-screen bg-bg flex">
      <div className="flex-1 flex items-center justify-center px-6 py-12">
        <div className="w-full max-w-sm">
          <div className="mb-8 flex items-center justify-between">
            ...
            <Link to="/" className="inline-flex items-center gap-2 rounded-lg border border-line px-3 py-2 text-xs text-ink-muted hover:bg-panel-hover hover:text-ink transition"><Home size={15} /> Back to Home</Link>
          </div>
          ...
          <div className="space-y-3 mb-5">
            ...
            <div className="flex items-center gap-3"><div className="h-px flex-1 bg-line"/><span className="text-[11px] uppercase tracking-wider text-ink-faint">or continue with password</span><div className="h-px flex-1 bg-line"/></div>
          </div>

          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-xs font-mono text-ink-muted mb-1.5">Username / Student ID / Email</label>
      ...
      <div className="hidden lg:flex lg:w-[45%] relative overflow-hidden bg-panel-dark items-center justify-center">
        ...
        <div className="relative z-10 flex flex-col items-center px-10 text-center">
          <div className="relative mb-8 animate-float"><ViewfinderFrame active cornerSize={22} className="relative w-56 h-56 overflow-hidden rounded-2xl bg-white/5 border border-white/10 backdrop-blur-sm flex items-center justify-center"><img src="/images/images.jpg" alt="Face recognition" className="absolute inset-0 h-full w-full object-cover opacity-90" /><div className="absolute inset-0 bg-panel-dark/20" /><div className="absolute inset-0 rounded-full border border-accent-glow/50 animate-pulse-ring" /><div className="absolute left-4 right-4 top-1/2 h-px bg-accent-glow/80 shadow-[0_0_12px_rgba(217,154,76,0.9)] animate-scan" /><div className="absolute inset-0 bg-gradient-to-b from-transparent via-accent-glow/5 to-transparent animate-pulse" /><ScanFace className="relative z-10 text-white/80" size={36} strokeWidth={1.2} /></ViewfinderFrame></div>
