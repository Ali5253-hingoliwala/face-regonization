...
import { LockKeyhole, MailCheck, ShieldCheck, Smartphone, Chrome, Clock3, AlertCircle, CheckCircle2 } from "lucide-react";
...
export default function SecurityPage() {
  ...
  const toggle2FA = async () => {
    setMessage(""); setError("");
    try { await api.post(security?.two_factor_enabled ? "/account/2fa/disable" : "/account/2fa/enable"); await load(); setMessage(security?.two_factor_enabled ? "2FA disabled." : "2FA enabled."); }
  ...
  const Status = ({ ok, children }: { ok: boolean; children: React.ReactNode }) => <span className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-medium ${ok ? "bg-emerald-500/10 text-emerald-600" : "bg-amber-500/10 text-amber-600"}`}>{ok ? <CheckCircle2 size={14}/> : <AlertCircle size={14}/>} {children}</span>;
  return <main className="mx-auto max-w-5xl p-4 sm:p-6 lg:p-8">
    ...
    <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      ...
      <div className="rounded-2xl border border-line bg-panel p-5"><Chrome className="text-accent"/><p className="mt-4 text-sm font-semibold">Google account</p><div className="mt-3"><Status ok={!!security?.google_linked}>{security?.google_linked ? "Linked" : "Not linked"}</Status></div></div>
