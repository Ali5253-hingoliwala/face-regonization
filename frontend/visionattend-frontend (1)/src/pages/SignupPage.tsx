...
export default function SignupPage() {
  ...
  async function handleGoogleStart(credential: string) {
    ...
    try {
      ...
      if (!result.onboardingRequired) { navigate(result.role === "admin" ? "/admin" : "/student"); return; }
      sessionStorage.setItem("va_google_credential", credential); sessionStorage.setItem("va_google_email", result.email ?? ""); sessionStorage.setItem("va_google_name", result.suggestedName ?? "");
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (googleMode) {
        ...
        sessionStorage.removeItem("va_google_credential"); sessionStorage.removeItem("va_google_email"); sessionStorage.removeItem("va_google_name");
        navigate(role === "admin" ? "/admin" : "/student");
      } else {
        await signup(studentId.trim(), name.trim(), password, faceImage); setSuccess(true); setTimeout(() => navigate("/login"), 1500);
      }
  ...
  const subtitle = googleMode ? "Add your student details, password, gender and face registration." : "Register your student details and capture one clear face.";
