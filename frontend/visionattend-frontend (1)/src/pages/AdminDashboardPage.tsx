...
import { Activity, CalendarClock, CheckCircle2, Users, XCircle } from "lucide-react";
      ...
      <main className="mx-auto max-w-[1600px] px-5 py-8 sm:px-8 lg:px-10">
        <section className="mb-7 flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <h2 className="mt-1 font-display text-3xl font-semibold sm:text-4xl">Attendance Overview</h2>
            <p className="mt-2 text-sm text-ink-muted">Track live sessions, scheduled lectures and attendance performance.</p>
          </div>
          <div className="flex items-center gap-2 rounded-xl border border-line bg-panel px-4 py-2.5 text-xs text-ink-muted shadow-sm">
            <span className={`h-2.5 w-2.5 rounded-full ${pipelineRunning ? "animate-pulse bg-green-500" : "bg-ink-faint"}`} />
        ...
        <section className="mt-6 grid gap-5 lg:grid-cols-[1.35fr_.65fr]">
          <div className="rounded-2xl border border-line bg-panel p-6 shadow-sm">
            <div className="flex items-center justify-between">
              <div><p className="font-mono text-[10px] uppercase tracking-[0.18em] text-accent">Live Analytics</p><h3 className="mt-1 text-lg font-semibold">Current Session Breakdown</h3></div>
              <span className={`rounded-full px-3 py-1 text-xs ${session ? "bg-green-50 text-green-700" : "bg-panel-hover text-ink-muted"}`}>{session ? "LIVE" : "IDLE"}</span>
            </div>
            <div className="mt-7 space-y-5"><Bar label="Present" value={present} total={max}/><Bar label="Late" value={late} total={max}/><Bar label="Absent" value={absent} total={max}/></div>
          ...
          <div className="rounded-2xl border border-line bg-panel p-6 shadow-sm">
            ...
            <Link to="/admin/schedule" className="mt-6 inline-flex items-center gap-2 rounded-xl bg-accent px-4 py-2.5 text-sm font-medium text-white"><CalendarClock size={16}/> Manage Schedule</Link>
          </div>
        </section>
