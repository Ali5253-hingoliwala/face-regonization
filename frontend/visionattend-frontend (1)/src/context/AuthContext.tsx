...
interface AuthContextValue extends AuthState {
  ...
  theme: Theme;
  toggleTheme: () => void;
  login: (username: string, password: string, captchaToken: string) => Promise<Role>;
...
function persistAuth(response: any) {
  const { access_token, role, name, student_id } = response.data ?? response;
  localStorage.setItem("va_token", access_token);
  localStorage.setItem("va_role", role);
  if (name) localStorage.setItem("va_name", name); else localStorage.removeItem("va_name");
  if (student_id) localStorage.setItem("va_student_id", student_id); else localStorage.removeItem("va_student_id");
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  async function login(username:string,password:string,captchaToken:string):Promise<Role>{ const response=await api.post("/auth/login",{username,password,captcha_token:captchaToken}); const next = persistAuth(response); setState(next); return next.role; }
  ...
  return <AuthContext.Provider value={{...state,isAuthenticated:!!state.token,authReady,theme,toggleTheme:()=>setTheme(t=>t==="light"?"dark":"light"),login,signup,googleLogin,googleOnboard,logout}}>{children}</AuthContext.Provider>;
}
