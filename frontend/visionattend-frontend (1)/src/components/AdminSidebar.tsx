...
const pageTitles: Record<string, string> = {
  "/admin": "Dashboard",
  "/admin/live-session": "Live Session",
      ...
      <header className="fixed inset-x-0 top-0 z-[60] border-b border-line bg-bg/90 backdrop-blur-xl transition-[left] duration-300" style={{ left: open ? "18rem" : "0" }}>
        <div className="flex h-16 items-center gap-3 px-4 sm:px-6">
          <button
            ...
            className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-line bg-panel text-ink-muted shadow-sm transition hover:-translate-y-0.5 hover:bg-panel-hover hover:text-ink"
          >
            {open ? <X size={18} /> : <MoreHorizontal size={20} />}
          </button>

          <div className="min-w-0">
            <p className="hidden font-mono text-[9px] uppercase tracking-[0.2em] text-accent sm:block">VisionAttend · Admin Portal</p>
            <h1 className="truncate font-display text-lg font-semibold sm:text-xl">{title}</h1>
          </div>

          <div className="ml-auto flex items-center gap-2">
            <div className="hidden items-center gap-2 rounded-full border border-line bg-panel px-3 py-1.5 text-[11px] text-ink-muted md:flex">
            ...
            <button
              ...
              className="flex h-10 w-10 items-center justify-center rounded-xl border border-line bg-panel text-ink-muted transition hover:bg-panel-hover hover:text-ink"
            >
      ...
      >
        ...
        <div className="border-t border-line p-4">
          ...
          <button
            ...
            className="flex w-full items-center gap-3 rounded-xl px-4 py-3 text-sm text-ink-muted transition hover:bg-red-50 hover:text-red-600"
          >
            <LogOut size={18} />
            Logout
          </button>
...
function ConfirmDialog({ title, text, onCancel, onConfirm }: { title: string; text: string; onCancel: () => void; onConfirm: () => void }) {
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/45 p-5 backdrop-blur-sm" onMouseDown={onCancel}>
      <div className="w-full max-w-sm rounded-2xl border border-line bg-panel p-6 shadow-2xl" onMouseDown={e => e.stopPropagation()}>
        ...
        <p className="mt-2 text-center text-sm text-ink-muted">{text}</p>
        <div className="mt-6 flex justify-end gap-2">
          <button onClick={onCancel} className="rounded-lg px-4 py-2 text-sm text-ink-muted hover:bg-panel-hover">Cancel</button>
