...
export default function AdminSidebar() {
  ...
  return <>
    ...
    <aside className={`fixed inset-y-0 left-0 z-50 flex w-64 flex-col border-r border-line bg-gradient-to-b from-white via-[#f5f7fa] to-[#dbe7f4] shadow-2xl transition-transform duration-300 ease-out dark:from-[#08111f] dark:via-[#10243a] dark:to-[#1e4568] ${open ? "translate-x-0" : "-translate-x-full"}`}>
      <div className="border-b border-line px-5 pb-5 pt-20"><p className="font-mono text-[9px] uppercase tracking-[0.2em] text-accent">VisionAttend · AI Attendance</p><p className="mt-4 text-sm text-ink-muted">Welcome back</p><p className="mt-0.5 font-display text-xl font-semibold">{name || "Administrator"}</p><p className="mt-1 text-xs text-ink-muted">Administrator</p></div>
      <nav className="flex-1 space-y-1 overflow-y-auto px-3 py-5">{links.map(({ label, to, icon: Icon }) => <NavLink key={to} to={to} end={to === "/admin"} onClick={() => setOpen(false)} className={({ isActive }) => `group flex items-center gap-3 rounded-xl px-4 py-3 text-sm transition-all duration-200 ${isActive ? "bg-accent text-white shadow-sm" : "text-ink-muted hover:translate-x-1 hover:bg-white/60 hover:text-ink dark:hover:bg-white/10 dark:hover:text-white"}`}><Icon size={18} className="transition-transform group-hover:scale-110"/><span>{label}</span></NavLink>)}</nav>
      <div className="border-t border-line p-4"><button onClick={toggleTheme} className="mb-2 flex w-full items-center justify-between rounded-xl px-4 py-3 text-sm text-ink-muted transition hover:bg-white/60 hover:text-ink dark:hover:bg-white/10 dark:hover:text-white"><span className="flex items-center gap-3">{theme === "dark" ? <Moon size={18}/> : <Sun size={18}/>} {theme === "dark" ? "Dark mode" : "Light mode"}</span><span className="text-xs">Switch</span></button><button onClick={() => setConfirmLogout(true)} className="flex w-full items-center gap-3 rounded-xl px-4 py-3 text-sm text-ink-muted transition hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-500/15 dark:hover:text-red-300"><LogOut size={18}/>Logout</button></div>
    </aside>
