...
function Check({checked,setChecked,children}:{checked:boolean;setChecked:(v:boolean)=>void;children:ReactNode}) { return <label className="flex cursor-pointer items-start gap-3 rounded-xl border border-line bg-bg p-3"><input type="checkbox" checked={checked} onChange={e=>setChecked(e.target.checked)} className="mt-1 h-4 w-4"/><span className="text-sm leading-5 text-ink-muted">{children}</span></label>; }
