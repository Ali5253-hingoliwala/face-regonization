import { BarChart3, CalendarDays, FileText, LayoutDashboard, LogOut, MoreHorizontal, Moon, Sun, UserRound, X, ChevronDown } from "lucide-react";
...
const links = [
  ...
  { label: "Attendance", to: "/student/attendance", icon: BarChart3 },
  { label: "Calendar", to: "/student/calendar", icon: CalendarDays },
  { label: "Leave", to: "/student/leave", icon: FileText },
...
export default function StudentSidebar() {
  ...
  const title = useMemo(() => pageTitles[location.pathname] ?? "Student Portal", [location.pathname]);
  ...
  useEffect(() => {
    const close = (event: MouseEvent) => { if (!(event.target as HTMLElement)?.closest?.("[data-profile-menu]")) setProfileOpen(false); };
  ...
  return <>
      ...
      <div className="flex h-16 items-center gap-3 px-4 sm:px-6">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <div className="relative hidden sm:block" data-profile-menu>
            ...
            {profileOpen && <div className="absolute right-0 top-[calc(100%+8px)] w-48 overflow-hidden rounded-xl border border-line bg-panel p-1.5 shadow-xl"><NavLink to="/profile" onClick={() => setProfileOpen(false)} className="flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm text-ink-muted hover:bg-panel-hover hover:text-ink"><UserRound size={16}/>Profile</NavLink><button onClick={() => setConfirmLogout(true)} className="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm text-ink-muted hover:bg-red-50 hover:text-red-600"><LogOut size={16}/>Logout</button></div>}
          </div>
        </div>
      </div>
    </header>
    ...
    <aside className={`fixed inset-y-0 left-0 z-50 flex w-64 flex-col border-r border-line bg-panel shadow-2xl transition-transform duration-300 ease-out ${open ? "translate-x-0" : "-translate-x-full"}`}>
      <div className="border-b border-line px-5 pb-5 pt-20"><p className="font-mono text-[9px] uppercase tracking-[0.2em] text-accent">VisionAttend · AI Attendance</p><p className="mt-4 text-sm font-medium text-ink-muted">Welcome back</p><p className="mt-0.5 font-display text-xl font-semibold text-ink">{name || "Student"}</p><p className="mt-1 text-xs text-ink-faint">Student</p></div>
      <nav className="flex-1 space-y-1 overflow-y-auto px-3 py-5">{links.map(({ label, to, icon: Icon }) => <NavLink key={to} to={to} end={to === "/student"} onClick={() => setOpen(false)} className={({ isActive }) => `group flex items-center gap-3 rounded-xl px-4 py-3 text-sm font-medium transition-all duration-200 ${isActive ? "bg-accent text-white shadow-lg shadow-accent/25" : "text-ink-muted hover:translate-x-1 hover:bg-panel-hover hover:text-ink"}`}><Icon size={18} className="transition-transform group-hover:scale-110"/><span>{label}</span></NavLink>)}</nav>
      <div className="border-t border-line p-4"><button onClick={toggleTheme} className="mb-2 flex w-full items-center justify-between rounded-xl px-4 py-3 text-sm font-medium text-ink-muted transition hover:bg-panel-hover hover:text-ink"><span className="flex items-center gap-3">{theme === "dark" ? <Moon size={18}/> : <Sun size={18}/>} {theme === "dark" ? "Dark mode" : "Light mode"}</span><span className="text-xs">Switch</span></button><button onClick={() => setConfirmLogout(true)} className="flex w-full items-center gap-3 rounded-xl px-4 py-3 text-sm font-medium text-ink-muted transition hover:bg-red-500/10 hover:text-red-500"><LogOut size={18}/>Logout</button></div>
    </aside>
    {confirmLogout && <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/45 p-5 backdrop-blur-sm"><div className="w-full max-w-sm rounded-2xl border border-line bg-panel p-6 shadow-2xl"><div className="mx-auto flex h-12 w-12 items-center justify-center rounded-2xl bg-accent-soft text-accent"><LogOut size={22}/></div><h2 className="mt-4 text-center text-lg font-semibold">Confirm Sign Out</h2><p className="mt-2 text-center text-sm text-ink-muted">Are you sure you want to sign out?</p><div className="mt-6 flex justify-end gap-2"><button onClick={() => setConfirmLogout(false)} className="rounded-lg px-4 py-2 text-sm text-ink-muted hover:bg-panel-hover">Cancel</button><button onClick={logout} className="rounded-lg bg-red-500 px-4 py-2 text-sm font-medium text-white">Sign Out</button></div></div></div>}
  </>;
}
