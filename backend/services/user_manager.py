...
class UserManager:
    ...
    The account document is intentionally forward-compatible with the upcoming
    Google OAuth, email verification, profile photo, and account-security work.
    Existing accounts are migrated lazily when UserManager starts, so adding
    these fields does not invalidate the current login/signup flow.
    """
    ...
    def _ensure_schema(self):
        """Backfill the account fields needed by the new account architecture."""
        now = datetime.now(timezone.utc)

        # Only fill fields that are missing. This keeps existing user data intact.
    ...
    def create_user(
        ...
        """Create an account while preserving compatibility with old callers."""
        now = datetime.now(timezone.utc)
    ...
    def mark_login(self, username):
        """Record the most recent successful login without changing credentials."""
        result = self.collection.update_one(
            {"username": username},
