    ...
    try:
        while True:
            ...
            if session is not None:
                ...
                # Start exactly one local pipeline for the active session.
                ...
                # A remote admin can end the session. The worker owns the
                # local camera process, so it is responsible for stopping it.
                if pipeline_process is not None and pipeline_process.poll() is None:
                    stop_pipeline(pipeline_process)
                pipeline_process = None
                active_session_id = None

            # If the pipeline crashed while the session is still active, the
            # next iteration restarts it automatically.
            if (
                session is not None
